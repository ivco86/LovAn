"""
API routes for YouTube video operations (Video App)
"""
...
from shared import PHOTOS_DIR, DATA_DIR, DATABASE_PATH
...
@videos_bp.route('/api/videos/<int:video_id>/file')
...
@videos_bp.route('/api/images/<int:image_id>/file')
def serve_image_file(image_id):
    """Serve video/image file by image_id"""
    video = db.get_youtube_video_by_image_id(image_id)
    if video:
        full_video = db.get_youtube_video(video['id'])
        filepath = full_video.get('filepath') if full_video else None
    else:
        image = db.get_image(image_id)
        filepath = image.get('filepath') if image else None
...
def get_subtitles_vtt(image_id):
    """Get subtitles as VTT file for HTML5 video player"""
...
def export_transcript(image_id):
    ...
    video_title = full_video.get('title', 'transcript') if full_video else 'transcript'
                ...
                for line in lines:
                    if line.startswith('Translation:'):
                        ...
                        for alt in alts.split(','):
                            alt = alt.strip()
                            if alt and alt.lower() != word.lower():
                                if not any(t['translation'] == alt for t in translations):
                                    translations.append({
                                        'source': 'AI',
                                        'translation': alt,
                                        'is_ai': True
...
def generate_video_highlight(image_id):
    """Generate an AI-powered highlight video"""
    try:
        data = request.json or {}
...
def serve_keyframe(filename):
    ...
    from shared import KEYFRAMES_DIR
